...
>
> **后续**:本篇的 schema 在 [16](16-spike-results.md) 落地为可编译 Rust + 真实 benchmark,然后在 [17](17-complete-memory-system.md) 被升级——因果表从"一张独立表"变成"一张 typed-edge 图的一类边",事实/时序/因果/共现统一在同一个激活扩散引擎上。
...
> **2026-07 更新**:本节默认了"外部检索器从因果图召回"是唯一方案。但 [13-reconstructive-memory.md](13-reconstructive-memory.md) §0 指出至少还有三种架构:(a) agent 自检索 + 系统提示词目录(把因果链目录放系统提示词,绕开元遗忘,详见 13 §1.2),(b) 重构式记忆(存因果碎片让 LLM 重构,天然任务感知,详见 13 §2),(c) 四层混合。本节是其中一种,不是唯一答案。
...
## 8.5 开放方向:因果图的跨 agent 共享协议

> **2026-07 新增。** 这节回应 [10](10-memory-frameworks.md) §1 的 MemOS MIP(Memory Interchange Protocol)—— 跨 LLM 共享记忆的协议。MIP 引出一个问题:如果记忆要跨模型/跨 agent 共享,那**因果图**应该怎么共享?

### MemOS MIP 暴露的问题

MemOS([10](10-memory-frameworks.md) §1)押注一个跨公司的记忆交换协议 —— 不同 LLM 之间共享记忆单元。这是一个有远见的方向:既然 LLM 是无状态([09](09-stateless-function.md) §1),记忆必然在外部;那不同模型/agent 之间迟早需要共享记忆,否则记忆碎片化。

但 MIP 共享的是**扁平记忆**(事实、偏好),不是**因果图**。这留下一个开放问题:

> **如果 agent A 学到了"决策 X 导致了结果 Y",agent B 怎么继承这条因果知识?**

### 因果图共享的三个层次

| 层次 | 共享什么 | 难度 | 价值 |
|---|---|---|---|
| **L1: 共享 CausalEdge** | 单条决策→结果边 | 低 | 一个 agent 的教训传给另一个 |
| **L2: 共享 MetaCausalEdge** | 跨任务的决策模式 | 中 | 一个 agent 的策略迁移到另一个 |
| **L3: 共享完整因果图** | agent 的全部经验因果结构 | 高 | agent 的"人格"迁移(对应 [07](07-philosophy-deep-dive.md) §4 身份迁移) |

**L1 是 MIP 的自然延伸** —— 把 CausalEdge 作为一种"记忆单元"通过 MIP 传输。难点是因果置信度(§3 步骤三)的跨 agent 校准 —— agent A 用 `temporal` 邻近判断的因果(confidence=0.3),agent B 接收时该怎么解读?

**L3 最有意思也最危险**。它对应 [07](07-philosophy-deep-dive.md) §4 的 Parfit 身份迁移 —— 把一个 agent 的因果链完整传给另一个 agent,等同于"复制人格"。这有伦理问题(agent 同一性),也有技术问题(因果图是 agent-specific 的还是 task-specific 的?)。

### 因果图的"本地性"问题

向量 embedding 是**通用**的(同一个事实在不同 agent 里有相同的向量),所以 MIP 共享扁平记忆是自然的。但因果图是**本地**的:

- agent A 的决策"用 Redis 做缓存"在 A 的因果图里是 `[决策:用 Redis] → [结果:缓存击穿]`
- agent B 从来没用过 Redis,这条因果边对 B 没意义

**因果图共享不是"传输",是"翻译"** —— 要把 A 在 A 的上下文里的因果知识,翻译成 B 在 B 的上下文里能用的形式。这需要某种**因果抽象**(从"用 Redis 导致缓存击穿"抽象成"未保护 stampede 导致击穿"),然后 B 在自己的上下文里重新实例化。

这是当前所有记忆公司都没碰的问题,也是 [11](11-causal-state-store.md) 因果状态库的真正开放方向。**MemOS 的 MIP 给出了问题(跨模型共享),因果状态库给出了数据结构(CausalEdge),但两者之间的"翻译层"完全空白。**
...
>
> 诚实的边界:它只能到 Pearl 因果阶梯第 1 层(关联),第 3 层(反事实)对 agent 基本不可能。但拥有因果数据结构,已经比当前的扁平事件流和无因果语义的实体关系图强一个维度。
>
> 2026-07 新增的开放方向:**因果图的跨 agent 共享**(§8.5)。MemOS 的 MIP 协议([10](10-memory-frameworks.md) §1)提出了跨模型共享记忆,但因果图的共享需要"翻译层"(因果抽象 + 本地实例化),当前完全空白。这是因果状态库从"单 agent 优化"走向"多 agent 协作"的关键一步。
>
> **这是 7×24 AGI 记忆架构里,从"知道缺什么"到"知道怎么建"的关键一步。**
